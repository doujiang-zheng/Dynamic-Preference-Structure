...
from args_config import gumbel_args_config
...
from util import EarlyStopMonitor, RandEdgeSampler, set_logger

#import numba

# Argument and global variables
try:
    parser = gumbel_args_config()
...
def eval_one_epoch(hint, dps, src, dst, ts, label):
    with torch.no_grad():
        ...
        for k in range(num_test_batch):
            ...
            prob_score = dps.forward(src_l_cut, dst_l_cut, ts_l_cut).sigmoid()
...
dps = GumbelGAN(train_ngh_finder,
                n_feat,
                e_feat,
...
if True:
    ...
    for epoch in epoch_bar:
        ...
        dps.ngh_finder = train_ngh_finder
        np.random.shuffle(idx_list)
        ...
        for k in batch_bar:
            ...
            optimizer.zero_grad()
            dps = dps.train()
            pos_prob, neg_prob = dps.contrast(src_l_cut,
                                              dst_l_cut,
                                              dst_l_fake,
                                              ts_l_cut,
                                              NUM_NEIGHBORS,
        ...
        val_acc, val_ap, val_f1, val_auc = eval_one_epoch(
            'val for old nodes', dps, val_src_l, val_dst_l, val_ts_l,
            val_label_l)
        epoch_bar.update()
...
if True:
    ...
    optimizer = torch.optim.Adam(dps.parameters(),
