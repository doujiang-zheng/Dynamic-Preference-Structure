...
class PosEncode(torch.nn.Module):
    def __init__(self, expand_dim, seq_len):
        ...
        self.pos_embeddings = nn.Embedding(num_embeddings=seq_len,
                                           embedding_dim=expand_dim)
...
class AttnModel(torch.nn.Module):
    ...
    def forward(self, src, src_t, seq, seq_t, seq_e, mask):
        ...
        output, attn = self.multi_head_target(
            q=q, k=k, v=k,
            mask=mask)  # output: [B, 1, D + Dt], attn: [B, 1, N]
...
class TGAN(torch.nn.Module):
    def __init__(self,
        ...
        if agg_method == 'attn':
            ...
            self.attn_model_list = torch.nn.ModuleList([
                LSTMPool(self.n_feat_dim, self.e_feat_dim, self.time_dim)
                for _ in range(num_layers)
            ])
        elif agg_method == 'mean':
            self.logger.info('Aggregation uses constant mean model')
            self.attn_model_list = torch.nn.ModuleList([
                MeanPool(self.n_feat_dim, self.e_feat_dim)
                for _ in range(num_layers)
            ])
        else:
        ...
        if use_time == 'time':
            ...
            self.time_encoder = PosEncode(expand_dim=self.time_dim,
                                          seq_len=seq_len)
        elif use_time == 'empty':
