...
class NeighborFinder:
    ...
    def find_before(self, src_idx, cut_time, norm=False):
        ...
        neighbors_idx = node_idx_l[off_set_l[src_idx]:off_set_l[src_idx + 1]]
        neighbors_ts = node_ts_l[off_set_l[src_idx]:off_set_l[src_idx + 1]]
        neighbors_norm_ts = norm_ts_l[off_set_l[src_idx]:off_set_l[src_idx +
                                                                   1]]
        neighbors_e_idx = edge_idx_l[off_set_l[src_idx]:off_set_l[src_idx + 1]]
    ...
    def exp_sampling(self, src_idx_l, cut_time_l, num_neighbors=20):
        ...
        for i, (src_idx, cut_time) in enumerate(zip(src_idx_l, cut_time_l)):
            ...
            ngh_idx, ngh_eidx, ngh_ts, norm_ts = self.find_before(src_idx,
                                                                  cut_time,
                                                                  norm=True)
            if len(ngh_idx) <= 0:
                continue
            ...
            sampled_idx = np.random.choice(len(ngh_ts),
                                           size=num,
                                           replace=False,
                                           p=prob)
            ...
            result = (out_ngh_node_batch[i], out_ngh_t_batch[i],
                      out_ngh_eidx_batch[i])
    ...
    def find_k_hop(self, k, src_idx_l, cut_time_l, num_neighbors=20):
        ...
        for _ in range(k - 1):
            ...
            (
                ...
                out_ngh_eidx_batch,
                out_ngh_t_batch,
            ) = self.get_temporal_neighbor(ngn_node_est, ngn_t_est,
                                           num_neighbors)
