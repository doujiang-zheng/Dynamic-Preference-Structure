...
def load_data(dataset="ia-contact", mode="format", root_dir="data/"):
...
def _iterate_datasets(dataset="all", mode="test_data", root_dir="data/"):
    if dataset != "all":
        if isinstance(dataset, str):
            ...
            return dataset
    fname = [
        f for f in os.listdir(os.path.join(root_dir, mode))
        if f.endswith(".edges")
    ]
...
def load_split_edges(dataset="ia-contact", root_dir="data/"):
    train_edges, nodes = load_data(dataset=dataset,
                                   mode="train",
                                   root_dir=root_dir)
    valid_edges, _ = load_data(dataset=dataset,
                               mode="valid",
                               root_dir=root_dir)
...
def load_label_edges(dataset="ia-contact", root_dir="data/"):
    train_edges, nodes = load_data(dataset=dataset,
                                   mode="label_train",
                                   root_dir=root_dir)
    valid_edges, _ = load_data(dataset=dataset,
                               mode="label_valid",
                               root_dir=root_dir)
    test_edges, _ = load_data(dataset=dataset,
                              mode="label_test",
                              root_dir=root_dir)
    return train_edges, valid_edges, test_edges, nodes
